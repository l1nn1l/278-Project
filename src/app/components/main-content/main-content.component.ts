...
export class MainContentComponent {
  ...
  moveToTrash(id: string): void {
    this.documentService
      ...
      .subscribe({
        ...
        error: (error) => {
          console.error(
            `Error occurred when trying to move the document to trash:`,
            error
          );
        },
      });
  ...
  handleItemMouseDown(event: MouseEvent, item: DocumentDTO): void {
    ...
    if (event.ctrlKey || event.metaKey) {
      ...
      console.log(
        'New selection set:',
        this.selectedItems.map((it) => it._id)
      );
    }
  ...
  toggleStarred(document: DocumentDTO) {
    ...
    this.documentService
      .updateDocumentStarStatus(document._id, document.starred)
      .subscribe({
        next: (response) => {
  ...
  getDocuments() {
    this.isLoading = true;
    this.documentService
      .getOwnedDocuments(localStorage.getItem('id'))
      .subscribe(
        (response) => {
          ...
          console.log(
            'These are the Documents from the database',
            this.documents
          );
        },
        (error) => {
  ...
  openRenameDialog(document: DocumentDTO): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        const updatedDocumentIndex = this.documents.findIndex(
  ...
  deleteSelectedDocuments(): void {
    console.log(
      'Starting deletion of selected documents:',
      this.selectedItems.map((item) => item._id)
    );
    ...
    const deletionRequests = this.selectedItems.map((item: DocumentDTO) =>
      this.documentService.softDeleteDocument(item._id).pipe(
        catchError((error: any) => {
          console.error(
            `Failed to delete document with ID ${item._id}:`,
            error
          );
          return of(null);
        })
    ...
    forkJoin(deletionRequests).subscribe((results: any[]) => {
      ...
      results.forEach((result, index: number) => {
        if (result !== null) {
          ...
          console.log(
            `Document with ID ${itemId} deleted successfully`,
            result
          );
        } else {
          console.log(
            `Failed to delete document with ID ${this.selectedItems[index]._id}`
          );
        }
      });

      // Update documents after deletion
      const remainingDocs = this.documents.filter(
        (doc) => !this.selectedItems.some((item) => item._id === doc._id)
      );
      console.log(
        'Documents remaining after deletion:',
        remainingDocs.map((doc) => doc._id)
      );
